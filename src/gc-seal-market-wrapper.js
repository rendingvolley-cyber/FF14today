...
const MARKET_CACHE_SCHEMA = "sell-through-300-top3-v3";
...
async function buildRecommendations(env) {
  ...
  return {
    ...
    ranking_mode: "velocity_hard_gate_3day_then_value",
    ...
    message: recommendations.length
      ? `約${GC_SEAL_SELL_BATCH_QUANTITY}個を${GC_SEAL_MAX_BATCH_DAYS}日以内に吸収できる実売速度を満たす候補だけを、1日実売数の多い順に表示しています。`
      : `約${GC_SEAL_SELL_BATCH_QUANTITY}個を${GC_SEAL_MAX_BATCH_DAYS}日以内に吸収できる候補を確認できませんでした。無理に遅い品は表示しません。`
  };
}
