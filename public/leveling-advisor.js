...
function battleMethods(job){
 if(job.role==="limited")return[method(1,"青魔道士は通常ルートから分離","青魔法の修得状況で最適解が変わるため、自動で通常ダンジョンを勧めません。","青魔専用",["未修得の重要青魔法を先に確認","レベル上げだけなら自分より高いフィールド敵を対象にする","協力者がいる場合はパワーレベリングを使う"],"レベル80到達")];
 if(job.level<15)return[method(1,"クラス／ジョブクエスト＋討伐手帳","レベル15までをまず作る。","今やる",["受注可能なクラス／ジョブクエストを完了","討伐手帳の未達成対象を上から処理","レベル15になったらサスタシャへ切り替え"],"レベル15到達")];
 const d=dungeonForLevel(job.level);const queue=job.role==="dps"?"攻撃役は待ち時間が長ければ、対応ダンジョンをコンテンツサポーターで即開始。コンテンツファインダーで申請するなら、待ち時間だけフィールドの突発イベントを1個ずつ処理。":"コンテンツファインダーで申請。タンク／ヒーラーは待ち時間が短いことが多いのでダンジョン周回を優先。";
 const steps=[`コンテンツファインダーで「${d.name}」を開く`,queue,"1周クリアする",`レベル${job.level+1}になったらジョブを再選択し、次の適正ダンジョンへ更新する`];
 const primary=method(1,`${d.name}を1周 → レベルが上がるまで繰り返す`,`レベル${job.level}で経験値が入る中で、現在レベル以下の最高レベルのレベリングダンジョンを具体指定。`,`主力`,steps,`レベル${job.level+1}到達`);
 const rows=[primary];
 if(job.role==="dps")rows.push(method(2,"待ち時間だけフィールドの突発イベントを処理","ダンジョンをやめて周回するのではなく、待ち時間を空白にしない用途。","待ち時間用",["ダンジョンをコンテンツファインダーで申請","現在地の適正な突発イベントを1つ処理","マッチングしたら即ダンジョンへ戻る"],"ダンジョン突入"));
 if(job.code==="SMN"||job.code==="SCH")rows.unshift(method(0,"召喚士／学者はレベル共有","どちらで経験値を稼いでも同じ巴術士レベルが上がる。","固有",["使いたい方で上記ダンジョンへ行く"],"共有レベル上昇"));
 return rows;
}
function crafterMethods(job){
 const band=job.level>=90?"黄金エリア":job.level>=80?"暁月エリア":"現在レベル帯";
 return[
  method(1,"収集品を『今のレベル以下で一番高いもの』に固定して納品",`${job.name} レベル${job.level}は、何を作るか探し回らず収集品窓口の最上位対象を1種類に固定する。`,`主力`,["収集品取引窓口を開く",`${job.name}の一覧でレベル${job.level}以下の最上位対象を1つ選ぶ`,`製作手帳でその品だけをお気に入り登録`,`必要素材をまとめて用意し、最低納品ライン以上の収集価値で連続製作`,`収集品取引窓口へまとめて納品`,`レベルが上がったら一覧を開き直し、より上位が解禁された時だけ品を変更`],`レベル${job.level+1}到達`),
...
function gathererMethods(job){
 if(job.role==="fisher")return[
  method(1,"オーシャンフィッシングに参加","開催枠が合うなら、漁師の経験値稼ぎをこれに固定する。","主力",["リムサ・ロミンサ：下甲板層 座標3.0, 12.7へ移動","オーシャンフィッシング受付に話しかけて『航路を確認』","出航受付中なら参加申請","餌が無ければ受付横のショップで使用する餌を購入","3海域を最後まで釣る","帰港後、次の出航まで2時間あるので別作業へ切り替える"],"1航海完了。次の開催枠まで別作業"),
  method(2,"開催待ちは釣り手帳の未取得を埋める","何を釣るか探す時間を減らすため、未取得マークが付いた穴を上から埋める。","待ち時間用",["釣り手帳を開く","現在レベル以下の未取得魚がある釣り場を選択","マップ表示して現地へ移動","未取得が消えるまで釣る"],"次のオーシャン受付開始")
 ];
 return[
  method(1,"収集品を『今のレベル以下で一番高いもの』に固定して採る",`${job.name} レベル${job.level}は、収集品取引の最上位対象1種類だけを回す。`,`主力`,["収集品取引窓口でギャザラー欄を開く",`${job.name}でレベル${job.level}以下の最上位対象を1つ選ぶ`,`採集手帳でその素材を検索し『採集場所を表示』`,`現地で収集品採集を有効にして必要数を採る`,`収集品取引窓口へまとめて納品`,`レベルが上がったら最上位対象だけ更新`],`レベル${job.level+1}到達`),
  method(2,"時限対象なら下の『時限採集を取得』を使う","時間条件がある素材だけは現在時刻から候補を自動表示する。","補助",["時限採集を取得を押す","『いま採れる』を優先","表示された場所・座標へ移動して採集"],"必要数採集")
 ];
}
export function levelingRecommendations(rawJob){const job=jobInfo(rawJob);if(!job)return{job:null,methods:[]};if(job.level>=job.cap)return{job,methods:[method(0,"レベル上限到達","このジョブはレベル上げ不要。","完了",[],`レベル${job.cap}`)]};const methods=job.group==="battle"?battleMethods(job):job.group==="crafter"?crafterMethods(job):gathererMethods(job);return{job,methods}}
