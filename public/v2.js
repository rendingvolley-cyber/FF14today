...
const JP_TEXT_REPLACEMENTS=[
  ...
  ["Trust","フェイス"],
  ["Dryskthota","オーシャンフィッシング受付"],
  ["Merchant & Mender","受付横のショップ"],
  ["Versatile Lure","万能ルアー"],
  ["Lodestone","ロードストーン"]
];
function jpText(value){let text=String(value??"");for(const[from,to]of JP_TEXT_REPLACEMENTS)text=text.split(from).join(to);return text.replace(/X:\s*([0-9.]+)\s*Y:\s*([0-9.]+)/g,"座標$1, $2").replace(/\bRank\s*(\d+)/g,"開拓ランク$1").replace(/\bLv\s*(\d+)/g,"レベル$1").replace(/\bEXP\b/g,"経験値").replace(/\bDPS\b/g,"攻撃役").replace(/\bCF\b/g,"コンテンツファインダー").replace(/\bID\b/g,"ダンジョン").replace(/\bFATE\b/g,"フィールドイベント").replace(/\bET\b/g,"エオルゼア時間").replace(/\bVI\b/g,"6").replace(/\bIV\b/g,"4").replace(/\bIII\b/g,"3").replace(/\bII\b/g,"2").replace(/\bV\b/g,"5").replace(/\bI\b/g,"1")}
...
$("syncButton")?.addEventListener("click",async()=>{const b=$("syncButton");setBusy(b,true,"同期中…");try{await api("/api/sync",{method:"POST"});await loadProfile();$("status").textContent="ロードストーン同期が完了しました。"}catch(error){$("status").textContent=`同期失敗：${error.message}`}finally{setBusy(b,false,"ロードストーン同期")}});
